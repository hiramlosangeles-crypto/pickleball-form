...
function validateStep1() {
    ...
    if (!vipChoice) {
        alert('Please choose Priority Alerts or Sunday Only');
        return false;
    }
...
async function handleFormSubmit(e) {
    ...
    // Validate all Step 1 fields (includes VIP if selected)
    if (!validateStep1()) {
        return;
    }
...
function showConfirmation(formData) {
    ...
    // Show confirmation (Step 2 of 2)
    const confirmation = document.getElementById('confirmation');
    confirmation.style.display = 'block';
    
    // Update progress bar to Step 2 of 2
    currentStep = 2;
    updateProgressBar();
